...
# 💎 Global CSS Design (Restored & Fixed)
st.markdown("""
    <style>
...
# FIX: Navigation function without rerun inside callback
if "current_page" not in st.session_state: st.session_state.current_page = "HOME"

def go_to(page):
    st.session_state.current_page = page
    # No st.rerun() here to avoid the error!
...
# --- ➕ 4. ADD RETAILER ---
    ...
    up = st.file_uploader("Excel Format: Name, PRM, Details, DUSE, ADVANCE", type=["xlsx","csv"])
    if up:
        ...
        if st.button("🚀 Process Bulk Upload"):
            ...
            for i, row in df_up.iterrows():
                ...
                if b_name:
                    ...
                    time.sleep(0.5)
                    if b_dues > 0: sess.post(WEBHOOK_URL, json={"action":"add_txn","date":date.today().strftime("%d-%m-%Y"),"r_name":b_name,"r_mob":b_mob,"type":"Opening Dues","qty":0,"amt_out":b_dues,"amt_in":0,"fse":"SYSTEM","txn_id":"OPENING"})
            ...
            if (f_n=="Avdhesh Kumar" and f_p=="9557") or (f_n=="Babloo kumar singh" and f_p=="2081"):
                ...
                for i, row in df_j.iterrows():
                    ...
                    if prm in prm_mapping:
                        ...
                        sess.post(WEBHOOK_URL, json={"action":"add_txn","date":date.today().strftime("%d-%m-%Y"),"r_name":prm_mapping[prm]['Name'],"r_mob":prm_mapping[prm]['Mobile'],"type":"Etop Transfer","qty":0,"amt_out":round(amt*0.97,2),"amt_in":0,"fse":f_n,"txn_id":str(row.get("Order ID",""))})
                        time.sleep(0.5)
