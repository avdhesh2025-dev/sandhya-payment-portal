...
# 💎 FULL SCREEN SUCCESS POPUP (Hang Free)
...
# 🛑 JAVASCRIPT: SMART CONTINUOUS GPS FETCH (No Hang)
st.components.v1.html("""
    ...
    function fetchGPS() {
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function(pos) {
                ...
                inputs.forEach(inp => {
                    if(inp.getAttribute('aria-label') === '📍 Live Location (Auto GPS)') {
                        if(inp.value === "") { // Only fill if empty!
            ...
            }, function(err) { console.log(err); }, { enableHighAccuracy: true, timeout: 5000 });
        }
    }
    // Check every 3 seconds until location is grabbed
...
# --- 📍 BEAT TRACKING (IN/OUT) ---
    ...
    with t1:
        st.info("Start your market day. App is waiting for location...")
        with st.form("form_in"):
            loc_in = st.text_input("📍 Live Location (Auto GPS)", key="in_loc", placeholder="Waiting for GPS...")
            if st.form_submit_button("Save PUNCH IN", type="secondary"):
                if loc_in:
                    time_str = datetime.now().strftime("%I:%M %p")
                    log_type = f"IN ({time_str}) | {loc_in}"
                    try:
                        requests.post(WEBHOOK_URL, json={"action":"add_txn","date":date.today().strftime("%d-%m-%Y"),"r_name":st.session_state.emp_name,"r_mob":st.session_state.emp_mob,"type":log_type,"qty":0,"amt_out":0,"amt_in":0,"fse":"System","txn_id":"BEAT"}, timeout=10)
                        st.session_state.success_display_text = "PUNCH IN Recorded!"
                        st.session_state.success_txn_type = log_type
                        st.session_state.show_success_modal = True
                        st.cache_data.clear(); st.rerun()
                    except: st.error("⚠️ Server error. Please try again.")
                else: st.error("⚠️ Please wait for location to load or Allow Location Access in browser!")
                    
    with t2:
        st.warning("End your day. App is waiting for location...")
        with st.form("form_out"):
            loc_out = st.text_input("📍 Live Location (Auto GPS)", key="out_loc", placeholder="Waiting for GPS...")
            if st.form_submit_button("Save PUNCH OUT & Calc KM", type="secondary"):
                if loc_out:
                    ...
                    try:
                        requests.post(WEBHOOK_URL, json={"action":"add_txn","date":today_str,"r_name":st.session_state.emp_name,"r_mob":st.session_state.emp_mob,"type":log_type,"qty":total_km,"amt_out":0,"amt_in":0,"fse":"System","txn_id":"BEAT"}, timeout=10)
                        st.session_state.success_display_text = f"PUNCH OUT: {total_km} KM"
                        st.session_state.success_txn_type = log_type
                        st.session_state.show_success_modal = True
                        st.cache_data.clear(); st.rerun()
                    except: st.error("⚠️ Server error. Please try again.")
        ...
        with st.expander("⚙️ Edit / Delete Entry"):
            if rows:
                ...
                if col_del.button("🗑️ Delete", use_container_width=True):
                    try:
                        requests.post(WEBHOOK_URL, json={"action":"delete_txn","r_name":name,"details":sel_e}, timeout=10)
                        st.success("Delete command sent to Sheet!")
                        st.cache_data.clear(); st.rerun()
                    except: st.error("⚠️ Timeout Error")
            ...
            with st.form("d_f"):
                ...
                if st.form_submit_button("Save", type="secondary"):
                    if verify_pin(f, p):
                        ...
                        try:    
                            requests.post(WEBHOOK_URL, json={"action":"add_txn","date":date.today().strftime("%d-%m-%Y"),"r_name":name,"r_mob":mob,"type":t,"qty":input_qty,"amt_out":final_amt,"amt_in":0,"fse":f,"txn_id":"KB"}, timeout=10)
                            ...
                            msg = f"*Retailer:* {name} ({prm})\n*Aapka dues hai:* ₹ {abs(cur_bal + final_amt):,.0f}\n*Aaj hi online ya cash de kar apna dues clear kare.*\n\n*Recent Transaction:*\n🗓 {date.today().strftime('%d-%m-%Y')} | {detail_str} | Diye (-): ₹{final_amt if final_amt>0 else input_qty}\n\nRegards,\n*SANDHYA ENTERPRISES*\nAVDHESH KUMAR\n📞 7479584179"
                            st.session_state.success_wa_link = f"https://wa.me/91{mob}?text={urllib.parse.quote(msg)}"
                            st.session_state.show_success_modal = True
                            st.cache_data.clear(); st.rerun()
                        except: st.error("⚠️ Connection Timeout. Google Sheet might be slow.")
        ...
        if st.session_state.kb_action == "mile":
            with st.form("m_f"):
                ...
                if st.form_submit_button("Save", type="secondary"):
                    if verify_pin(f, p):
                        try:
                            requests.post(WEBHOOK_URL, json={"action":"add_txn","date":date.today().strftime("%d-%m-%Y"),"r_name":name,"r_mob":mob,"type":f"Payment ({m})","amt_in":a,"fse":f,"txn_id":"KB"}, timeout=10)
                            st.session_state.success_display_text = f"₹ {a:,.0f}"; st.session_state.success_txn_type = f"Payment Received ({m})"
                            msg = f"*Retailer:* {name} ({prm})\n*Aapka dues hai:* ₹ {abs(cur_bal - a):,.0f}\n*Payment ke liye Dhanyawad.*\n\n*Recent Transaction:*\n🗓 {date.today().strftime('%d-%m-%Y')} | Payment Received | Mile (+): ₹{a}\n\nRegards,\n*SANDHYA ENTERPRISES*\nAVDHESH KUMAR\n📞 7479584179"
                            st.session_state.success_wa_link = f"https://wa.me/91{mob}?text={urllib.parse.quote(msg)}"
                            st.session_state.show_success_modal = True
                            st.cache_data.clear(); st.rerun()
                        except: st.error("⚠️ Connection Timeout.")
...
# --- OTHER PAGES ---
        ...
        with t2:
            ...
            with st.form("bill"):
                ...
                if st.form_submit_button("Bill SIMs", type="secondary"):
                    if ap == "9557":
                        try:
                            requests.post(WEBHOOK_URL, json={"action":"add_txn","date":date.today().strftime("%d-%m-%Y"),"r_name":f,"r_mob":"0","type":"Sim Allocation","qty":q,"amt_out":0,"amt_in":0,"fse":"Admin","txn_id":"S"}, timeout=10)
    ...
    with st.form("add_ret"):
        ...
        if st.form_submit_button("Save Retailer", type="secondary"):
            try:
                requests.post(WEBHOOK_URL, json={"action":"add_retailer","name":n.upper(),"mobile":m,"prm":p,"location":l.upper(),"date":date.today().strftime("%d-%m-%Y")}, timeout=10)
                st.success("Retailer Added!"); st.cache_data.clear()
            except: st.error("⚠️ Server Timeout.")
    ...
    with st.form("entry_form"):
        ...
        if st.form_submit_button("Save Transaction", type="secondary"):
            if verify_pin(f_n, f_p):
                ...
                try:
                    requests.post(WEBHOOK_URL, json={"action":"add_txn","date":date.today().strftime("%d-%m-%Y"),"r_name":r_info['Retailer Name'],"r_mob":str(r_info['Mobile Number']).split('.')[0],"type":t_type,"qty":input_qty,"amt_out":final_amt if t_type in ["Etop Transfer", "JPB V4"] else 0,"amt_in":final_amt if t_type=="Payment Received" else 0,"fse":f_n,"txn_id":"E"}, timeout=10)
