...
def create_pdf(r_name, r_mob, bal, r_data):
    ...
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", 'B', 16); pdf.cell(0, 8, "Sandhya Enterprises", ln=True, align='C')
    pdf.set_font("Arial", 'B', 11); pdf.cell(0, 6, "Jio Authorities Distributor", ln=True, align='C')
    pdf.set_font("Arial", '', 9); pdf.cell(0, 5, "Rosera Rod Meghpatti City Samastipur Bihar 848117", ln=True, align='C')
    pdf.cell(0, 5, "GSTIN: 10GQZPK8313P1Z1 | Email: smp.sandhya02@gmail.com", ln=True, align='C')
    pdf.line(10, 40, 200, 40); pdf.ln(10)
    pdf.set_font("Arial", 'B', 12); pdf.cell(100, 6, f"Retailer: {r_name}"); pdf.cell(0, 6, f"Mobile: {r_mob}", ln=True, align='R')
    pdf.cell(0, 8, f"Balance: Rs {bal:,.2f}", ln=True); pdf.ln(5)
    pdf.set_font("Arial", 'B', 9); pdf.cell(25, 8, "Date", 1); pdf.cell(75, 8, "Particulars", 1); pdf.cell(30, 8, "Dr", 1); pdf.cell(30, 8, "Cr", 1); pdf.cell(30, 8, "Bal", 1); pdf.ln()
    pdf.set_font("Arial", '', 9)
    for r in r_data:
        pdf.cell(25, 8, str(r['date']), 1); pdf.cell(75, 8, str(r['item'])[:40], 1); pdf.cell(30, 8, str(r['debit']), 1); pdf.cell(30, 8, str(r['credit']), 1); pdf.cell(30, 8, str(r['bal']), 1); pdf.ln()
...
if st.session_state.current_page == "HOME":
    ...
    if st.session_state.role == "Admin": st.dataframe(inv_df, use_container_width=True)
        ...
        q_c = 'Qty'
        alloc = pd.to_numeric(led_df[(led_df['Retailer Name']==st.session_state.emp_name)&(led_df['Product/Service']=='Sim Allocation')][q_c], errors='coerce').sum()
        dist = pd.to_numeric(led_df[(led_df['FSE Name']==st.session_state.emp_name)&(led_df['Product/Service']=='Sim Card')][q_c], errors='coerce').sum()
    ...
    with st.form("add_r"):
        ...
        if st.form_submit_button("Save"):
            ...
            st.session_state.success_display_text = "Retailer Added"; st.session_state.show_success_modal = True; st.cache_data.clear(); st.rerun()
