...
if st.session_state.show_success_modal:
    ...
    st.markdown(f"""
    <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 60vh; background: #f0fdf4; border-radius: 20px; border: 2px solid #86efac; padding: 20px; margin-top: 20px;">
        <div style="font-size: 80px;">✅</div>
        <div style="font-size: 25px; font-weight: 800; color: #166534;">Success!</div>
...
if st.session_state.current_page == "HOME":
    ...
    with col2:
        if st.button("💰 Today Coll.", use_container_width=True): go_to("COL"); st.rerun()
        if st.button("📦 Entry", use_container_width=True): go_to("ENTRY"); st.rerun()
        if st.button("🚨 Urgent Rec.", use_container_width=True): go_to("URGENT"); st.rerun()
    ...
    if st.session_state.kb_retailer is None:
        ...
        for i in all_r:
            if sel == "All" or i['Disp'] == sel:
                ...
                with c2: st.markdown(f"<div class='{cls}'>₹ {abs(i['Bal']):,.0f}</div>", unsafe_allow_html=True)
    else:
        ...
        # Calculate current balance for reminder
        ...
        <div style='background:#0b57d0; color:white; padding:15px; border-radius:12px; margin-bottom:10px; text-align:center;'>
            ...
            <div style='display:flex; justify-content:space-around; margin-top:10px;'>
                ...
                <a href='https://wa.me/91{mob}?text=Namaste%2C%20Sandhya%20Enterprises%20se%20aapka%20Kul%20Bakaya%20Rs%20{abs(bal)}%20hai.%20Kripya%20jald%20bhugtan%20karein.' style='background:#25D366; color:white; padding:10px 20px; border-radius:8px; text-decoration:none; font-weight:800;'>📲 REMINDER</a>
            </div>
        </div>
        """, unsafe_allow_html=True)
        
        st.write(f"### Ledger: ₹ {bal:,.0f}")
        b1, b2 = st.columns(2)
        ...
        if st.session_state.kb_action == "mile":
            with st.form("m_f"):
                ...
                a = st.number_input("Amount Received", min_value=0.0)
                if st.form_submit_button("Save"):
