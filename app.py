...
st.markdown("""
    ...
    /* Button Wobble Effect for Entry */
    .wobble-btn > div > button {
        ...
        border-left: 6px solid #007bff !important; transition: 0.2s;
    }
                    ...
                    with st.form(f"col_{_}"):
                        ...
                        if st.form_submit_button("Save"):
                            if (f_n=="Avdhesh Kumar" and f_p=="9557") or (f_n=="Babloo kumar singh" and f_p=="2081"):
                                payload = {"action": "add_txn", "date": date.today().strftime("%d-%m-%Y"), "r_name": name, "r_mob": mob, "type": "Collection", "amt_in": p_amt, "amt_out": 0, "fse": f_n, "txn_id": "DIRECT"}
    ...
    if up:
        ...
        if st.button("🚀 Process Bulk Retailers"):
            ...
            for i, row in df_up.iterrows():
                ...
                b_prm, b_mob = str(row.get("PRM ID", "")).split('.')[0], str(row.get("DETAILS", "")).split('.')[0]
    ...
    if up_j:
        ...
        if st.button("🚀 Match & Upload"):
            if (f_n=="Avdhesh Kumar" and f_p=="9557") or (f_n=="Babloo kumar singh" and f_p=="2081"):
                ...
                st.success("Bulk Success!"); st.cache_data.clear()
            else: st.error("Wrong PIN")
...
# --- 🚨 7. URGENT RECOVERY ---
    ...
    f_n = st.selectbox("FSE", ["Babloo kumar singh", "Avdhesh Kumar"]); f_p = st.text_input("PIN", type="password")
    if (f_n=="Babloo kumar singh" and f_p=="2081") or (f_n=="Avdhesh Kumar" and f_p=="9557"):
        now = datetime.now(); overdue_list = []
        for _, row in led_df.iterrows():
            ...
            if is_u:
                overdue_list.append({"Retailer": row['Retailer Name'], "Amount": a_out-a_in, "Date": row['Date'], "Type": r_type})
                st.markdown(f"""<div class="urgent-card"><b>Retailer:</b> {row['Retailer Name']}<br><b>Dues:</b> ₹{a_out-a_in} | <b>Since:</b> {row['Date']}</div>""", unsafe_allow_html=True)
                with st.form(f"rsn_{_}"):
                    rsn = st.text_area("Reason for delay")
                    if st.form_submit_button("Submit Reason"):
                        requests.post(WEBHOOK_URL, json={"action":"add_txn","date":date.today().strftime("%d-%m-%Y"),"r_name":row['Retailer Name'],"type":"Urgent Remark","txn_id":rsn,"fse":f_n})
                        st.success("Reason Recorded!")
        
        if overdue_list:
            st.download_button("📥 Download Urgent List (Excel)", pd.DataFrame(overdue_list).to_csv(index=False).encode('utf-8-sig'), "Urgent_Recovery.csv", "text/csv", use_container_width=True)
