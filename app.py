...
# 💎 Restore CSS Design (English UI + 3D Effects)
st.markdown("""
    <style>
...
# 🛑 CONFIGURATION & LIVE DATABASE LINKS
...
# Pull Live Data with Cache Busting
@st.cache_data(ttl=5)
def load_data():
    try:
        cb = int(time.time())
...
# --- ➕ 4. ADD RETAILER & BULK RETAILERS ---
    ...
    st.markdown("---")
    st.header("📂 Bulk Retailer Upload (with Opening Balance)")
    ...
    if (f_p=="2081" and f_n=="Babloo kumar singh") or (f_p=="9557" and f_n=="Avdhesh Kumar"):
        ...
        for _, row in led_df.iterrows():
            ...
            if is_u:
                ...
                st.markdown(f"""<div class="urgent-card"><b>Retailer:</b> {row['Retailer Name']}<br><b>Amt:</b> ₹{a_out-a_in} | <b>Since:</b> {row['Date']} ({r_type})</div>""", unsafe_allow_html=True)
                with st.form(f"r_{_}"):
                    rsn = st.text_area("Why is this payment pending?")
                    if st.form_submit_button("Submit Reason"):
                        ...
                        st.success("Recorded!"); st.cache_data.clear()
        if u_list:
